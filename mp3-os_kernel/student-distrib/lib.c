...
void putc(uint8_t c, int term_id) {
    ...
    if(c == '\n' || c == '\r') {
        ...
        if(term_id == curr_term){
            ...
            *(uint8_t *)(video_mem + (term_id+1)*ONE_KB + ((NUM_COLS * terminals[term_id].term_y + terminals[term_id].term_x) << 1)) = c;
            *(uint8_t *)(video_mem + (term_id+1)*ONE_KB + ((NUM_COLS * terminals[term_id].term_y + terminals[term_id].term_x) << 1) + 1) = ATTRIB;
        }
...
void vert_scroll(int term_id) {
    ...
    if(term_id == curr_term){
        ...
        // just replace i with the x_offset location == NUM_COLS+i == width+i
        for (i = 0; i < (NUM_ROWS-1) * NUM_COLS; i++) {
            *(uint8_t *)(video_mem + (term_id+1)*ONE_KB + (i << 1)) = *(uint8_t *)(video_mem + (term_id+1)*ONE_KB + ((NUM_COLS + i) << 1));               // looks at next row's video memory
            *(uint8_t *)(video_mem + (term_id+1)*ONE_KB + (i << 1) + 1) = *(uint8_t *)(video_mem + (term_id+1)*ONE_KB + ((NUM_COLS + i) << 1) + 1);       // just looks at the next row's ATTRIB    
        }

        // clear last row - so start where we ended above
        // this is basically clear()
        for (i = (NUM_ROWS-1) * NUM_COLS; i < NUM_ROWS * NUM_COLS; i++) {
            *(uint8_t *)(video_mem + (term_id+1)*ONE_KB + (i << 1)) = ' ';
            *(uint8_t *)(video_mem + (term_id+1)*ONE_KB + (i << 1) + 1) = ATTRIB;
        }
    }
